...
//---------------------------------------------------------------------------------
	...
	u16* oam = (u16*)OAM;
	oam[0] = OBJ_Y(test) | OBJ_SHAPE(2);
	oam[1] = OBJ_X(test2) | ATTR1_SIZE_8;
	...
	while (1) 
	{
		...
		if(keys & KEY_UP)	 oam[0] = OBJ_Y(test--) | OBJ_SHAPE(2);
		if(keys & KEY_DOWN)	 oam[0] = OBJ_Y(test++) | OBJ_SHAPE(2);
		if(keys & KEY_RIGHT) oam[1] = OBJ_X(test2++) | ATTR1_SIZE_8;
		if(keys & KEY_LEFT)  oam[1] = OBJ_X(test2--) | ATTR1_SIZE_8;
